"""ui/views/benefits.py — moved verbatim out of ui/app.py on 2026-09-03."""
...
def benefits_benchmarking_page(catalog, benefits_svc):
    ...
    # Placed HERE, immediately above the compa-ratio, and not at the top of the
    # page. What follows divides an actual salary by a market P50 and shows the
    # result as a position out of 100 — and every field in the compensation
    # slot changes what that number means. At 49% bargaining coverage in
    # Germany there may be no collective scale to be positioned against at all;
    # at 92,09% in Spain a convenio floor almost certainly governs, so a
    # position below it is not a market gap but a floor breach. Where scales
