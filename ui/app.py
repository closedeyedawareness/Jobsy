        ...
        if not results_s:
            ...
            if not role_pool:
                ...
                for job_id, indices in role_pool.items():
                    ...
                    current_sk = {req.skill_id: req.required_level
                                  for req,_ in catalog.get_role_skills(job_id)}
                    try:
                        gaps_s = catalog.skill_gap(current_sk, target_id)
                    except Exception:
                        gaps_s = []
                    score_s  = readiness_score(gaps_s)
                    n_dev    = sum(1 for g in gaps_s if g["gap"]>0)
                    n_exc    = sum(1 for g in gaps_s if g["gap"]<0)
                    lbl_s, col_s = readiness_label(score_s)
                    from_job = catalog.repository.jobs.get(job_id)
                    if not from_job: continue
                    top_gaps = [g["skill_name"] for g in gaps_s if g["gap"]>0][:2]
                    candidates.append({
                        "current_role": from_job.standard_title,
                        "function":     from_job.function,
                        "level":        from_job.level,
                ...
                candidates.sort(key=lambda c: (-c["score"], c["n_develop"]))
...
def _build_org_json(df_input, results, title_col):
    """Build a D3-ready tree.

    Strategy: Company → Departments (collapsed) → Employees by level.
    When ManagerID gives a real multi-level hierarchy (>2 levels deep for
    most employees) we use actual reporting lines instead.
    """
    ...
    DEPT_COLORS  = {
        "Executive":"#17212E","Finance":"#0E7C66","HR":"#2B5FA6",
    ...
    def get_name(row):
        ...
        if nc_col:
            return str(row.get(nc_col,"")).strip()
        return ""
    ...
    for idx, row in df_input.iterrows():
        ...
        mid  = str(row[mgr_col]) if mgr_col and pd.notna(row.get(mgr_col)) else None
        ...
        raw_nodes[eid] = {
            "id": eid, "name": name, "input_title": ititle,
            "matched_title": mtitle, "level": level,
            "dept": dept, "confidence": conf,
            "manager_id": mid, "type": "employee",
            "color": LEVEL_COLORS.get(level, "#5A6B7A"),
            "children": [],
        }

    # ── decide: real hierarchy or dept-based? ─────────────────────────────
    use_real = False
    if mgr_col:
        valid_ids = set(raw_nodes.keys())
        mgr_counts = {}
        for n in raw_nodes.values():
            if n["manager_id"] and n["manager_id"] in valid_ids:
                mgr_counts[n["manager_id"]] = mgr_counts.get(n["manager_id"],0)+1
        if mgr_counts:
            top_share = max(mgr_counts.values()) / len(raw_nodes)
            use_real  = top_share < 0.45   # real hierarchy: no single person manages >45%

    # ── REAL hierarchy mode ───────────────────────────────────────────────
    if use_real:
        roots = []
        for eid, node in raw_nodes.items():
            mid = node["manager_id"]
            if mid and mid in raw_nodes:
                raw_nodes[mid]["children"].append(node)
            else:
                roots.append(node)
        if len(roots) == 1:
            return _json.dumps(roots[0], default=str)
        return _json.dumps({
            "id":"__root__","name":"Organisation","type":"root",
            "color":"#17212E","matched_title":"","level":"","dept":"",
            "children":roots
        }, default=str)

    # ── DEPT-BASED hierarchy mode ─────────────────────────────────────────
    # Group everyone by dept; within dept sort by level then name
    dept_groups = {}
    for n in raw_nodes.values():
    ...
    for dept in dept_groups:
        dept_groups[dept].sort(
            key=lambda x: (LEVEL_SORT.get(x["level"], 9), x["name"])
        )

    # Build department nodes (start COLLAPSED — children stored in _children)
    dept_nodes = []
    for dept_name in sorted(dept_groups.keys()):
        members = dept_groups[dept_name]
        dc = DEPT_COLORS.get(dept_name, "#5A6B7A")
        # top employee = first by level sort
        top = members[0] if members else None
        dept_node = {
            "id":    f"dept-{dept_name}",
            "name":  dept_name,
            "matched_title": (
                f"{top['name']} · {top['matched_title']}" if top else f"{len(members)} employees"
            ),
            "level": "", "dept": dept_name, "type": "department",
            "color": dc,
            "children":   [],          # starts collapsed — children loaded by D3 on click
            "_members": members,        # stored for JS to expand
        }
        dept_nodes.append(dept_node)

    # Embed members inside dept nodes so JS can expand them
    for dn in dept_nodes:
        dn["children"] = dn.pop("_members")

    # Find CEO / top executive as root if present
    ceo_node = None
    for dept_name, members in dept_groups.items():
        if dept_name.lower() in ("executive","c-suite","leadership"):
            # pick the CEO
            for m in members:
                if any(t in m["input_title"].lower() for t in ["ceo","chief executive"]):
                    ceo_node = m
                    break

    if ceo_node:
        # put remaining exec dept members as direct children of CEO, then dept nodes
        exec_dept = next((dn for dn in dept_nodes if dn["dept"].lower() in ("executive","c-suite")), None)
        other_execs = [m for m in (exec_dept["children"] if exec_dept else []) if m is not ceo_node]
        other_depts  = [dn for dn in dept_nodes if dn is not exec_dept]
        ceo_node["children"] = other_execs + other_depts
        tree = ceo_node
    else:
        tree = {
            "id":"__root__","name":"Organisation","type":"root",
            "color":"#17212E","matched_title":f"{len(raw_nodes)} employees",
...
// initial centering — fit root + dept nodes in view
svg.call(zoom.transform, d3.zoomIdentity.translate(60, H/2).scale(1.0));
</script>
</body>
</html>"""
