...
def main():
    ...
    # page navigation
    page = st.sidebar.radio("", ["Matching", "Skills Assessment", "Skill Gap", "Organisation", "Organigram"], label_visibility="collapsed")
...
def skill_assessment_page(catalog):
    ...
    if not upload_sa:
        ...
        if existing:
            st.success(f"✓ Assessments loaded for {len(existing)} people from previous upload.")
    ...
    # Auto-detect format: wide (skills as columns) or long (Name, SkillName, Level)
    lower_cols = [c.lower() for c in df_sa.columns]
    is_long = "skillname" in lower_cols or "skill_name" in lower_cols or "skill name" in lower_cols

    skill_names = {s.skill_name.lower(): s.skill_id for s in catalog.repository.skills.values()}
    assessments = {}  # {employee_key: {skill_id: level}}

    if is_long:
        # Long format: Name/EmployeeID, SkillName, Level
        id_col   = next((c for c in df_sa.columns if c.lower() in ["employeeid","id","name","employee"]), df_sa.columns[0])
        ...
        if not sk_col or not lv_col:
            st.error("Long format requires columns: [EmployeeID/Name], [SkillName], [Level]"); return
        for _, row in df_sa.iterrows():
            emp = str(row[id_col]).strip()
            sn  = str(row[sk_col]).strip().lower()
            sid = skill_names.get(sn)
            if not sid: continue
            try: lv = max(0, min(5, int(float(row[lv_col]))))
            except: continue
            assessments.setdefault(emp, {})[sid] = lv
    else:
        # Wide format: first 3 cols are metadata, rest are skill names
        id_col   = next((c for c in df_sa.columns if c.lower() in ["employeeid","id"]), df_sa.columns[0])
        name_col = next((c for c in df_sa.columns if "name" in c.lower()), df_sa.columns[min(1,len(df_sa.columns)-1)])
        skill_cols = [c for c in df_sa.columns if c.lower() in skill_names]
        if not skill_cols:
            st.warning("No recognised skill columns found. Check column names match the template exactly.")
            return
        for _, row in df_sa.iterrows():
            emp = str(row.get(id_col, row.get(name_col, "Unknown"))).strip()
            emp_skills = {}
            for col in skill_cols:
                val = row[col]
                if _pdsa.isna(val) or str(val).strip() == "": continue
                try: lv = max(0, min(5, int(float(val))))
                except: continue
                sid = skill_names[col.lower()]
                emp_skills[sid] = lv
            if emp_skills:
