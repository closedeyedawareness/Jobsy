...
def main():
    ...
    # page navigation
    page = st.sidebar.radio("", ["Matching", "Skill Gap", "Organisation"], label_visibility="collapsed")
...
def org_hierarchy_page(catalog):
    ...
    # build name lookup if available
    names = {}
    if df_input is not None and name_col and title_col:
        for idx, row in df_input.iterrows():
            if idx < len(results):
                names[idx] = str(row.get(name_col, "")).strip() or None

    # group by function → level
