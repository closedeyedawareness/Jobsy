...
def main():
    ...
    with st.sidebar:
        ...
        if _ps_available():
            ...
            st.markdown(
                '<div class="jobsy-secret-card">'
                '<div class="jobsy-secret-title">Session saving inactive</div>'
                '<div class="jobsy-secret-copy">Add <code>SUPABASE_URL</code> and '
                '<code>SUPABASE_KEY</code> in Streamlit secrets to enable resume codes.</div>'
                '</div>',
                unsafe_allow_html=True,
            )
