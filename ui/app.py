...
def main():
    ...
    # page navigation
    page = st.sidebar.radio("", ["Matching", "Skills Assessment", "Skill Gap", "9-Box Grid", "Organisation", "Organigram"], label_visibility="collapsed")
...
def nine_box_page(catalog):
    ...
    with tab_input:
        if not results:
            ...
            # Upload ratings CSV option
            upload_r = st.file_uploader("Upload ratings CSV (EmployeeID/Name, Performance 1-3, Potential 1-3)",
                                         type=["csv","xlsx"], key="nb_upload")
            if upload_r:
                df_r = _pd9.read_csv(upload_r) if upload_r.name.endswith(".csv") else _pd9.read_excel(upload_r)
                ic = next((c for c in df_r.columns if c.lower() in ["employeeid","id","name"]), df_r.columns[0])
                ...
                if pc and ptc:
                    ...
                    st.success(f"Loaded ratings for {len(ratings)} employees.")
                else:
                    st.error("Need columns: [ID/Name], [Performance], [Potential]")

            # Download template
            if results:
                emp_keys = [get_name(i) for i,r in enumerate(results) if r.matched][:10]
                tmpl_r = _pd9.DataFrame({"EmployeeID":emp_keys,"Performance (1-3)":"","Potential (1-3)":""})
                tbuf = _io9.BytesIO(); tmpl_r.to_excel(tbuf, index=False)
                st.download_button("⬇ Download rating template", tbuf.getvalue(),
                    file_name="jobsy_9box_template.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
            st.markdown(f'<div style="font-family:{FONT_MONO};font-size:11px;letter-spacing:.1em;'
                f'text-transform:uppercase;color:{C["muted"]};margin-bottom:10px">'
                f'Manual entry</div>', unsafe_allow_html=True)

            matched = [(i,r) for i,r in enumerate(results) if r.matched][:50]
            cols = st.columns(2)
            for idx, (i, r) in enumerate(matched):
                emp = get_name(i)
                existing = ratings.get(emp, (2,2))
                with cols[idx%2]:
                    with st.expander(f"{emp} — {r.standard_title}", expanded=False):
                        p  = st.select_slider("Performance",options=[1,2,3],
                                              value=existing[0],
                                              format_func=lambda x: PERF_LABELS[x],
                                              key=f"nb_p_{i}")
                        pt = st.select_slider("Potential",options=[1,2,3],
                                              value=existing[1],
                                              format_func=lambda x: POT_LABELS[x],
                                              key=f"nb_pt_{i}")
                        ratings[emp] = (p, pt)
