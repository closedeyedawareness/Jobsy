...
def main():
    ...
    with tab_upload:
        # Show last uploaded file info if present
        prev_df = st.session_state.get("upload_df")
        if prev_df is not None and not isinstance(prev_df, type(None)):
            prev_col = st.session_state.get("upload_title_col","")
            st.caption(f"↩ Last upload: {len(prev_df)} rows · title column: **{prev_col}**  —  upload a new file to replace.")

        upload = st.file_uploader("Upload CSV or Excel (.csv, .xls, .xlsx)",
                                   type=["csv","xls","xlsx"], key="main_uploader")
        if upload:
            try:
