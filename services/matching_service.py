...
class MatchingService:
    ...
    def _resolve(self, title: str) -> Optional[tuple[str, MatchType, int]]:
        ...
        if self.enable_fuzzy:
            ...
            if hit is not None:
                job_id, score = hit
                # Seniority guard: reject fuzzy matches where level is inconsistent
                # with clear seniority signals in the input title
                matched_job = getattr(self._index, "_get_job_level", lambda x: None)(job_id)
                if self._seniority_mismatch(title, job_id):
                    pass  # reject this fuzzy hit
                else:
    ...
    # seniority signals in title → expected minimum level
    _SENIOR_SIGNALS = {
        "chief ":       "Lead",   # "chief executive" etc — space avoids matching "mischief"
        "(ceo)":        "Lead", "(cto)": "Lead", "(cfo)": "Lead", "(coo)": "Lead",
        "director":     "Lead",
        "head of ":     "Lead",
        "vp ":          "Lead",   # "vp sales" etc
        "vice president": "Lead",
        "senior ":      "Senior", "sr. ": "Senior", "sr ": "Senior",
        "lead ":        "Senior", "principal ": "Senior",
    }
    _JUNIOR_SIGNALS = {"junior", "jr.", "jr ", "entry", "trainee", "stage", "intern",
                       "graduate", "assistant", "associate", "medewerker", "stagiair"}
    _LEVEL_ORDER   = {"Junior":1, "Medior":2, "Senior":3, "Lead":4}

    # Function domain keywords: title substring → set of allowed functions
    _FUNCTION_SIGNALS: dict[str, set[str]] = {
    ...
    def _seniority_mismatch(self, input_title: str, job_id: str) -> bool:
        """Return True if the fuzzy-matched role is the wrong level OR wrong function."""
        job = None
        try:
            repo = self._catalog.repository if hasattr(self._catalog, "repository") else None
            if repo:
        ...
        matched_level    = job.level
        matched_function = job.function
        t_lower = input_title.lower()

        # ── 1. Seniority check ────────────────────────────────────────────
        for signal, min_level in self._SENIOR_SIGNALS.items():
            if signal in t_lower:
                if self._LEVEL_ORDER.get(matched_level, 0) < self._LEVEL_ORDER.get(min_level, 3):
                    return True
        for signal in self._JUNIOR_SIGNALS:
            if signal in t_lower:
                if self._LEVEL_ORDER.get(matched_level, 0) >= self._LEVEL_ORDER.get("Senior", 3):
                    return True

        # ── 2. Function domain check ──────────────────────────────────────
        # Pad with spaces for whole-word matching
        t_padded = f" {t_lower} "
        for keyword, allowed_fns in self._FUNCTION_SIGNALS.items():
            if keyword in t_padded:
                if matched_function not in allowed_fns:
                    return True  # function mismatch → reject fuzzy hit
