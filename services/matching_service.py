...
class MatchingService:
    ...
    def _seniority_mismatch(self, input_title: str, job_id: str) -> bool:
        """Return True if the fuzzy-matched role's level is clearly wrong for this title."""
        from core.models import Job  # noqa
        # get matched job level
        job = None
        try:
            ...
            pass
        if not job:
            return False  # can't check — allow it through
        matched_level = job.level
        t_lower = input_title.lower()
        # Check for senior signals in input title — must match Lead or Senior level roles
        for signal, min_level in self._SENIOR_SIGNALS.items():
            if signal in t_lower:
                min_ord = self._LEVEL_ORDER.get(min_level, 3)
                matched_ord = self._LEVEL_ORDER.get(matched_level, 0)
                if matched_ord < min_ord:
                    return True  # mismatch — reject
        # Check for junior signals — must NOT match Lead/Senior roles
        for signal in self._JUNIOR_SIGNALS:
            if signal in t_lower:
                matched_ord = self._LEVEL_ORDER.get(matched_level, 0)
                if matched_ord >= self._LEVEL_ORDER.get("Senior", 3):
                    return True  # junior title → Lead role — reject
        return False
